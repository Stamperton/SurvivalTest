...
public class CraftingStation : StorageBase, IInteractable
{
    ...
    public void ConvertResource()
    {
        ...
        if (!UtilityInventory.CheckSameUseThenEmpty(inventoryEntries, currentRecipe.resourceToCraft, out InventoryEntry _output))
            return;
        ...
        foreach (ResourceCost _resourceCost in currentRecipe.costOfRecipe)
        {
            ...
            for (int i = 0; i < _cost; i++)
            {
                ...
                else if (UtilityInventory.FindResource(inventoryEntries, _resourceCost.materialType, out InventoryEntry _entryAlt))
                {
                    UtilityInventory.DecrementInventorySlot(_entryAlt);
                }
            }
        }
        //Generate Resource
